class IssuesController < ApplicationController
  ...
  def setup_push_notification
    ...
    # Start jobs in the background (doesn't seem to do it as a background job)
    # view_context.start_delayed_jobs
    redirect_to admin_push_notifications_path, notice: "Push notifications are being created as a delayed job - next set the delayed jobs running."
  end
        ...
        if p["productId"].include?("single")
          ...
          if p["productId"].include?("#{@issue.number}single")
            ...
            begin
              ...
              logger.warn "Google Play product verification failed for issue #{@issue.id}: #{e.class} #{e.message}"
              next
            end

            if result.purchase_state == 0
              logger.info "Google Play: VALID purchase: #{result.order_id}"
          ...
          begin
            ...
            logger.warn "Google Play subscription verification failed for issue #{@issue.id}: #{e.class} #{e.message}"
            next
          end
